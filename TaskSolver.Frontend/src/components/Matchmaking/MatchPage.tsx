...
const MatchContent = ({ matchId }: { matchId: string }) => {
  ...
  const loadLanguages = useCallback(async () => {
    ...
    try {
      ...
      if (languagesResponse.data.length > 0 && !language) {
  ...
  useEffect(() => {
    ...
    const generateTemplate = () => {
      if (!currentTask.input || currentTask.input.length === 0) {
    ...
    return input;
      ...
      const params = currentTask.input.map((input) => input.name).join(", ");
      const comments = currentTask.input
        .map(
          (input) =>
    ...
    // ${
      currentTask.output
        ? `Ожидаемый вывод: ${currentTask.output}`
        : "Определите формат вывода"
    }
    \n    // Ваше решение здесь\n    \n    return ${
      currentTask.output ? "ожидаемый_результат" : "result"
    };
}`;
    };
  ...
  useEffect(() => {
    loadMatch();
    loadLanguages();
  }, [loadMatch, loadLanguages]);

  useEffect(() => {
    if (code) {
      localStorage.setItem(`task:${matchId}:${language}`, code);
    }
  }, [code]);

  useEffect(() => {
    const code = localStorage.getItem(`match:${matchId}:${language}`);
  ...
  useEffect(() => {
    if (language) {
      localStorage.setItem(`match:${matchId}`, language);
    }
  }, [language]);

  useEffect(() => {
    const language = localStorage.getItem(`match:${matchId}`);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-6">
              ...
              <button
                ...
                disabled={!language || isSubmitting || !!match.endedAt}
            ...
            <div className="bg-[#2a2a2a] rounded-xl p-6 border border-[#333333] shadow-lg">
              <div className="flex items-center justify-between">
                ...
                <select
                  ...
                  disabled={languages.length === 0 || !!match.endedAt}
                >
              ...
              <div className="bg-[#2d2d2d] px-6 py-4 text-sm font-medium border-b border-[#333333] flex justify-between items-center">
                ...
                {language && (
                  <span className="text-gray-400 text-sm font-mono">
                    ...
                    {languages.find((l) => l.id === language)?.version}
                  </span>
                )}
              </div>
              <div className="space-y-4">
